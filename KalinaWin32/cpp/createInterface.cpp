...
LRESULT CALLBACK msgWinProc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		break;
	case WM_PREPARED:
		if(WORKMODE_ENCRYPTION==workMode.direction)
			hThread = CreateThread(NULL,0,encryptStart,NULL,0,NULL);
		if(WORKMODE_DECRYPTION==workMode.direction)
...
int showAbout(){
	MessageBox(hwndMainWindow,"Crypter / decrypter on Kalina algorythm by DNS","Kalina crypto for win32",MB_OK);
