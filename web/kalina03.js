function Kalina(b,k){
    ...
    I.expandKey=function(){
		var i,j,c,a,A=[],B=[],C=[],Ks=[],o=(I.k==2*I.b)?I.c:0;
        ...
        var rotatedkey;
		for(j=0;j<I.c;j++){
			q[0][j] = [0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00];
			q[1][j] = [0x02,0x00,0x02,0x00,0x02,0x00,0x02,0x00];
			q[2][j] = [0x04,0x00,0x04,0x00,0x04,0x00,0x04,0x00];
			q[3][j] = [0x08,0x00,0x08,0x00,0x08,0x00,0x08,0x00];
			q[4][j] = [0x10,0x00,0x10,0x00,0x10,0x00,0x10,0x00];
			q[5][j] = [0x20,0x00,0x20,0x00,0x20,0x00,0x20,0x00];
			q[6][j] = [0x40,0x00,0x40,0x00,0x40,0x00,0x40,0x00];
			q[7][j] = [0x80,0x00,0x80,0x00,0x80,0x00,0x80,0x00];
			q[8][j] = [0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01];
			q[9][j] = [0x00,0x02,0x00,0x02,0x00,0x02,0x00,0x02];
		}
    ...
    //console.log(I.keyToString(rotatedkey[4])) ;return;   
		...
		for(n=0;n<=I.r;n+=2)
		{
			...
			for(j=0;j<I.c;j++){
				c=0;Kr_tmp[j]=[];
				for(i=0;i<8;i++){
					Kr_tmp[j][i] = q[n/2][j][i] + Ks[j][i] + c;//fi
					if(Kr_tmp[j][i] > 255){
						Kr_tmp[j][i] &= 255;
						c = 1;
					}else c = 0;
			...
			for(j=0;j<I.c;j++){
				c=0;
				for(i=0;i<8;i++){
					A[j][i] = Kr_tmp[j][i] + rotatedkey[n][j][i] + c;//eta
					if(A[j][i] > 255){
						A[j][i] &= 255;
						c = 1;
					}else c = 0;
					A[j][i] = I.S[i][A[j][i]];// pi
					B[(j+I.shift[i])&(I.c_1)][i]= A[j][i];//tau
				}
			}
			...
			for(j=0;j<I.c;j++)
				for(i=0;i<8;i++)
				{
					a = 0;
					for(k=0;k<8;k++)
						a ^= I.M(I.v[(k-i+8)&7],B[j][k]);
					A[j][i] = I.S[i][(a & 255)^Kr_tmp[j][i]];//+kappa-fi+pi
					C[(j+I.shift[i])&(I.c_1)][i]= A[j][i];//tau
				}
			...
			for(j=0;j<I.c;j++){
				...
				for(i=0;i<8;i++)
				{
					a = 0;
					for(k=0;k<8;k++)
						a ^= I.M(I.v[(k-i+8)&7],C[j][k]);

					A[j][i] = (a & 255) + Kr_tmp[j][i] + c;//+eta
					if(A[j][i] > 255){
						A[j][i] &= 255;
						c = 1;
					}else c = 0;
    ...
    function encrypt(){//transform state matrix prepared by other functions
		...
		for(round = 1;round < I.r; round++)
		{
			...
			for(j=0;j<I.c;j++)
				for(i=0;i<8;i++)
				{
					a = 0;
					for(k=0;k<8;k++)
						a ^= I.M(I.v[(k-i+8)&7],Gtmp[j][k]);
					I.G[j][i] = (a & 255)^(I.Kr[round][j][i]);
				}
		}
		//pi+tau
		for(j=0;j<I.c;j++)
		...
		for(j=0;j<I.c;j++){
			...
			for(i=0;i<8;i++)
			{
				...
				I.G[j][i] = (a & 255) + I.Kr[round][j][i] + c;
				if(I.G[j][i] > 255){
					I.G[j][i] &= 255;
					c = 1;
				}else c = 0;
    ...
    function decrypt(){//transform state matrix
		...
		for(j=0;j<I.c;j++)
			for(i=0;i<8;i++){
				a = 0;
				for(k=0;k<8;k++)
					a ^= I.M(I._v[(k-i+8)&7],I.G[j][k]);
				Gtmp[(j-I.shift[i]+I.c)&(I.c_1)][i] = I.Q[i][(a & 255)];
			}
    ...
    I.transportDecode=function(s){
        ...
        // console.log(k);
        for(i=0;i<k;i+=4){
            d = I.T.indexOf(s.charAt(i));
            e = I.T.indexOf(s.charAt(i+1));
            f = I.T.indexOf(s.charAt(i+2));
            g = I.T.indexOf(s.charAt(i+3));
