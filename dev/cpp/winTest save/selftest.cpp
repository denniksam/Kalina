...
extern lpfnexpandKey expandKey;
extern lpfncrypt encrypt,decrypt;
...
int selfTest(){
	...
	/*char str[100];
	sprintf(str,"%d %d %d %d %d %d %d %d %d %d ",sample[0].eStatus,sample[0].dStatus,
	sample[1].eStatus,sample[1].dStatus,sample[2].eStatus,sample[2].dStatus,sample[3].eStatus,sample[3].dStatus,
	sample[4].eStatus,sample[4].dStatus);
	MessageBox(NULL,str,"",IDOK);*/
	return 0;
}

int showTestResult(HWND hwnd){
	/*char *str = new char[128];
	sprintf(str,"Mode: %d / %d\nEncrypt test:  %s\nDecrypt test:  %s",
		sample[k].blockSize,sample[0].keySize,
		((sample[k].eStatus==1)?"PASSED":"FAILED"),
		((sample[k].dStatus==1)?"PASSED":"FAILED"));
	return MessageBox(hwnd,str,"Self test result",IDOK);
	*/
	char *str = new char[256];
	sprintf(str,"Mode\t\tEncrypt test\tDecrypt test\n128/128\t\t%s\t\t%s\n128/256\t\t%s\t\t%s\n256/256\t\t%s\t\t%s\n256/512\t\t%s\t\t%s\n512/512\t\t%s\t\t%s",
		(sample[0].eStatus==1)?"PASSED":"FAILED",(sample[0].dStatus==1)?"PASSED":"FAILED",
		(sample[1].eStatus==1)?"PASSED":"FAILED",(sample[1].dStatus==1)?"PASSED":"FAILED",
		(sample[2].eStatus==1)?"PASSED":"FAILED",(sample[2].dStatus==1)?"PASSED":"FAILED",
		(sample[3].eStatus==1)?"PASSED":"FAILED",(sample[3].dStatus==1)?"PASSED":"FAILED",
		(sample[4].eStatus==1)?"PASSED":"FAILED",(sample[4].dStatus==1)?"PASSED":"FAILED"
	);
	return MessageBox(hwnd,str,"Self test result",MB_OK);
}
