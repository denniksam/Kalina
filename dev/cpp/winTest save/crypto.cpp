...
int loadKey(HWND hwnd){
	...
	f.lpstrFilter = "All files\0*.*\0Text files\0*.TXT\0\0";
	f.nFilterIndex = 1;
...
DWORD WINAPI decryptGo(){
	...
	LONGLONG dBlocks = fDSize.QuadPart/blockBytes, step100 = dBlocks / 100, doneOld = 0, doneNew = 0;
	//if(FILE_WRONG_SIZE
	...
	doneNew += nRead;
	//sprintf(buf,"%I64d %I64d",fSize.QuadPart,fDSize.QuadPart);
	//MessageBox(hwndDecSrcFile,buf,"Size",MB_OK);
	...
	while(fDSize.QuadPart > blockBytes){
		...
		doneNew += nRead;
		for(i=doneOld/step100;i<doneNew/step100;i++)
			SendMessage(hwndProgressControl, PBM_STEPIT, 0, 0); 
		if(i>1)
			doneOld = doneNew;
	}
...
DWORD WINAPI decryptStart(void*){
	...
	LARGE_INTEGER fSize,fDSize;
	GetFileSizeEx(fS,&fDSize);
	LONGLONG /*dBlocks = fDSize.QuadPart/blockBytes,*/ fDSave = fDSize.QuadPart;
/*	
	if(0==buf)
			buf = new char[512];
		if(0==buf){
		...
		sprintf(buf,"fsize %I64d, blockBytes %I64d, dBlocks %I64d, step100 %I64d",fDSize.QuadPart,blockBytes,dBlocks,step100);
		MessageBox(hwndDecSrcFile,buf,"Decryption process",MB_YESNO|MB_ICONQUESTION);
SendMessage(msgWin,WM_COMMAND,IDC_THREADSTOP,0);return 0;	
	*/
