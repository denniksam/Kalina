...
int createInterface(HWND hwnd,HINSTANCE hInst){
	...
	// Help button
	tmp = 
		CreateWindow("Button","Help",
			WS_CHILD|WS_VISIBLE,
			60,330,75,23,
			hwnd,(HMENU)IDC_HLP,hInst,0
		);
	if(!tmp) 
		return -1;
...
LRESULT CALLBACK msgWinProc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_THREADSTOP:
			...
			sprintf(buf,"%ld",d);
			MessageBox(hwnd,buf,"GetExitCodeThread",MB_OK);
...
int showProgress(){
	...
	msgWin = CreateWindowEx(WS_EX_CLIENTEDGE,
		"msgWindow","Kalina crypto for win32",
		WS_VISIBLE|WS_OVERLAPPEDWINDOW&~(WS_THICKFRAME|WS_MAXIMIZEBOX),
		CW_USEDEFAULT,CW_USEDEFAULT,330,210, 
