...
extern "C"
{
	DLLEXPORT byte GFmul(byte a,byte b){
		if(a==0 || b==0) 
			return 0; 
		return pw2val[(pw2ind[a]+pw2ind[b])];
	}
	
	DLLEXPORT byte* expandKey(int blockSize, int keySize, const byte *rawKey, byte *destination = 0){
		byte C, R, C1;
	...
	DLLEXPORT byte* encrypt(int blockSize, int keySize, byte *initState, const byte *expandedKey, bool rewriteState = false){
		byte C, R, C1;
		byte shift[ROWLENGTH];
	...
	DLLEXPORT byte* decrypt(int blockSize, int keySize, byte *initState, const byte *expandedKey, bool rewriteState = false){
		byte C, R, C1;
		byte shift[ROWLENGTH];
		byte i,j,k,acc,carry_over;
		int n,s;
