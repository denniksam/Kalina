function Kupina( b ) {
	...
	I.transform0 = function( matrix = I.G ) {
		...
		for( let n = 0; n < I.t; ++n ) {
			for( j = 0; j < I.c; ++j ) {
				G[ j ][ 0  ] ^= ( j << 4 ) ^ n ;  // kappa
				for( i = 0; i < 8; ++i ) {
					Gtmp[ ( j + I.shift[ i ] ) & ( I.c - 1 ) ][ i ] = I.S[ i ][ G[ j ][ i ] ] ;						
				}
			}
			// psi
			for( j = 0; j < I.c; ++j ) {
	...
	I.transform1 = function( matrix = I.G ) {
		var Gtmp = [] ;
		var G = [] ;
		for( j = 0; j < I.c; ++j ) {
			Gtmp[ j ] = [] ;
		...
		for( let n = 0; n < I.t; ++n ) {
			for( j = 0; j < I.c; ++j ) {
				var dz = [ 243, 240, 240, 240, 240, 240, 240, ( ( ( I.c - 1 - j ) << 4 ) ^ n ) & 0xFF ] ;
				let carry_over = 0 ;
				for( i = 0; i < 8; ++i ) {
					G[ j ][ i ] = G[ j ][ i ] + dz[ i ] + carry_over ;
					if( G[ j ][ i ] > 0xFF ) {
						G[ j ][ i ] &= 0xFF ;
						carry_over = 1;
					}
	...
	I.hv = function( m = I.G ) {
		...
		for( j = 0; j < I.c; ++j ) {
			h[j] = [] ;
			for( i = 0; i < 8; ++i ) {
				h[j][i] = I.IV[j][i] ^ m[j][i] ;
			}
		}
		let t0 = I.transform0(h) ;
		let t1 = I.transform1() ;
		for( j = 0; j < I.c; ++j ) {
			for( i = 0; i < 8; ++i ) {
				h[j][i] = t0[j][i] ^ t1[j][i] ^ I.IV[j][i] ;
			}
		}
		return h ;
	}
